...
describe("a media condition the compiler cannot compile keeps its slot", () => {
  test("every media feature form compiles to a term, so no media prelude is dropped", () => {
    ...
    // A ratio DOES compile — `<ratio>` is a pair standing for its quotient, and
    // both runtimes derive the same number from their two axes — so this term
    // carries a value rather than the null marker. The point stands either way:
    // the prelude keeps its slot.
    expect(
      conditionsFor(`@media (min-aspect-ratio: 3/4) ${body}`),
    ).toStrictEqual([[[">=", "aspect-ratio", 0.75]]]);

    // The marker itself, on a feature value nothing can resolve at compile time.
    expect(
      conditionsFor(`@media (min-width: env(safe-area-inset-top)) ${body}`),
    ).toStrictEqual([[[">=", "width", null]]]);
  });
});
