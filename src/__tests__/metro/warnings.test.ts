...
/**
 * Fifteen declarations this compiler cannot fully express, which is more than
 * the summary's ten-entry cap so both arms of the cap are observable.
 *
 * Three of the fifteen do not land in the `properties` channel: `mix-blend-mode`
 * and `transform-origin` are expressible here and warn about nothing, and
 * `font-variant-numeric` is a property this compiler understands whose VALUE it
 * cannot render — so it reports under `values`. That mix is deliberate now that
 * it is there: it makes this the one case that proves a summary renders more
...
describe("formatCompilerWarnings", () => {
  ...
  test("a summary sorts, caps at ten and says how to see the rest", () => {
    expect(
      ...
      [
        "react-native-css: src/global.css - 13 declarations dropped, no React Native equivalent",
        "  properties: backdrop-filter, background-blend-mode, background-position, border-spacing, break-after, break-before, clear, order, resize, touch-action (+2 more)",
        "  values: font-variant-numeric: ordinal",
  ...
  test("verbose lists every entry and drops the hint", () => {
    expect(
      ...
      [
        "react-native-css: src/global.css - 13 declarations dropped, no React Native equivalent",
        "  properties: backdrop-filter, background-blend-mode, background-position, border-spacing, break-after, break-before, clear, order, resize, touch-action, white-space, will-change",
        "  values: font-variant-numeric: ordinal",
      ].join("\n"),
    );
  });
