...
describe("testMediaFeatureInterval", () => {
  ...
  const unanswerable: [
    ...
    // An unresolved bound is spelled `null`, not `undefined`:
    // `MediaFeatureOperand` is `Exclude<StyleDescriptor, undefined> | null`, so
    // the compiler can only ever put `null` in a bound slot and `undefined`
    // there is a type error rather than a case the runtime has to answer.
