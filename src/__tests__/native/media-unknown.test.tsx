...
describe("a negated term the runtime cannot measure does not apply", () => {
  ...
  test("not (width: env(safe-area-inset-top)) - an operand with no compile-time value", () => {
    // A ratio compiles to its quotient here, so the operand with no value is an
    // `env()` instead. lightningcss folds `not` into the operator for a range
    // feature, but a plain equality keeps it, so this is where a null operand
    // meets a negation.
    const component = renderAt(
      `${base}
       @media not (width: env(safe-area-inset-top)) { .my-class { color: blue; } }`,
      500,
      1000,
    );
