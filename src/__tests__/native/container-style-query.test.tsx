...
describe("other undecidable container terms are unknown, not false", () => {
  test("(width: env(safe-area-inset-top)) - an operand with no compile-time value", () => {
    // The container's width is measured; it is the right-hand side that never
    // arrives, because `env()` has no compile-time answer. A ratio is not this
    // case here — `<ratio>` compiles to its quotient and the axis is measured,
    // so `(aspect-ratio: 2)` is decided rather than refused.
    const child = renderContainer(
      `${base}
       @container (width: env(safe-area-inset-top)) { .child { color: blue; } }`,
      400,
      200,
    );
  ...
  test("not (width: env(safe-area-inset-top)) - a refusal under negation", () => {
    const child = renderContainer(
      `${base}
       @container not (width: env(safe-area-inset-top)) { .child { color: blue; } }`,
      500,
      200,
    );
