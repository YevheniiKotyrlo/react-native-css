...
describe("a light-dark() inside a pseudo-element", () => {
  /**
   * Each pseudo-element maps ONE declaration onto a prop of the host component:
   * `::selection` its `background-color` onto `selectionColor`, the band
   * painted behind the selected text, and `::placeholder` its `color` onto
   * `placeholderTextColor`. The dark branch arrives as its own rule, and it has
   * to be mapped the same way — an unmapped one is a plain declaration on the
   * element, which is the thing the pseudo-element was never asking about.
   */
  const cases = [
  ...
  test.each(cases)(
    ...
    (pseudoElement, cssProperty, prop) => {
      registerCSS(
        `.my-class${pseudoElement} { ${cssProperty}: light-dark(#f00, #00f); }`,
      );
