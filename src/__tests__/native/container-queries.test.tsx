...
describe("boolean features", () => {
  ...
  test("a feature the runtime cannot measure does not match", () => {
    // The feature has to be one the runtime genuinely cannot answer, and no
    // SIZE feature qualifies any more — `inline-size` and `block-size` resolve
    // to the physical axes, so every `ContainerSizeFeatureId` is measurable.
    // `getContainerFeatureValue`'s `default:` arm is still reachable because
    // lightningcss widens a container feature name to `string`, and that is
    // what this exercises: no value, so the boolean context is unknown rather
    // than false.
    registerCSS(`
      .container { container-name: my-container; }
      .child { color: red; }

      @container (fictional-feature) {
        .child { color: blue; }
      }
    `);
