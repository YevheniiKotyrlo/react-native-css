 ...
 *
 * Both operands are wider than `number`, because narrowing at the call site is
 * how the second copy gets written: an evaluator that has to reject a keyword
 * before it can call this ends up deciding `=` itself. The width is the union
 * of what can actually arrive — a MEASURED feature value is a
 * `StyleDescriptor` and is `undefined` when the evaluator could not measure it,
 * while a COMPILED bound is a `MediaFeatureOperand` and is `null` when the
 * compiler could not resolve it. `MediaInterval` puts a bound in a slot this
 * also reads a value from, so both spellings of "no answer" reach here.
 */
export function compareMediaFeature(
  operator: MediaFeatureComparison,
  left: StyleDescriptor | null,
  right: StyleDescriptor | null,
): boolean {
  ...
  // anything about a keyword. Neither spelling of "no answer" equals anything,
  // including the other and including itself: two operands nobody could
  // resolve are not thereby the same.
  if (operator === "=") {
